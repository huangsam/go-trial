...
func TestLogServerUnaryInfo(t *testing.T) {
	...
	info := &grpc.UnaryServerInfo{
		FullMethod: "/pkg.practical.Echoer/EchoOnce",
	}
	req := &pb.EchoRequest{Message: "Test Message"}
	ctx := context.Background()

	resp, err := practical.LogServerUnaryInfo(ctx, req, info, mockHandler)
	require.NoError(t, err)
...
func TestLogServerStreamInfo(t *testing.T) {
	...
	info := &grpc.StreamServerInfo{
		FullMethod:     "/pkg.practical.Echoer/EchoStream",
		IsClientStream: true,
		IsServerStream: true,
	}
	mockStream := &mockStream{}

	err := practical.LogServerStreamInfo(nil, mockStream, info, mockHandler)
	require.NoError(t, err)
}
